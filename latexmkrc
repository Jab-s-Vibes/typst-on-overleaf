...
$pdflatex = 'export LD_PRELOAD=/tmp/pollshim.so RAYON_NUM_THREADS=1; mkdir -p typst-bin; tar -zxf typst-x86_64-unknown-linux-gnu.tar.gz -C typst-bin --strip-components=1 2>> output.log; cp typst-bin/typst /tmp/typst 2>> output.log; cp pollshim.so /tmp/pollshim.so 2>> output.log; chmod +x /tmp/typst 2>> output.log; /tmp/typst compile --package-path ./typst-packages main.typ.txt output.pdf 2>> output.log; test -s output.pdf || /tmp/typst compile --package-path ./typst-packages main.typ.txt output.pdf 2>> output.log; test -s output.pdf || /tmp/typst compile --package-path ./typst-packages main.typ.txt output.pdf 2>> output.log; ls -la output.pdf >> output.log 2>&1; test -s output.pdf && echo COMPILE_OK >> output.log || echo COMPILE_FAILED >> output.log; perl synctex-gen.txt 2>> output.log | gzip -c > output.synctex.gz #';
